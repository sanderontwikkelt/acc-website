...
export async function AdminSideBar({
  ...
  const [
    ...
    [EntityEnum.PORTFOLIO, ActionEnum.FIND]
  )
  ...
  return (
    <Sidebar
      ...
      id='sidebar'
      className={cn(
        'max-md:fixed max-md:inset-0 max-md:w-screen transition-all max-md:-translate-x-full duration-300'
      )}
    >
      <SidebarItem className='flex items-center gap-1 pl-1 w-full relative'>
        <span className='text-xl font-bold sr-only'>KLEERLIJK</span>
        <svg
          width='52'
          height='52'
          viewBox='0 0 52 52'
      ...
      {(canFindCategories || canFindPortfolio) && (
        <SidebarItem>
          <SidebarItemLabel>Gegevens</SidebarItemLabel>
          <SidebarItemContent>
            {canFindPortfolio && (
              <SidebarItemLink href='/portfolio'>
                <Book className='mr-2 h-4 w-4' />
                Portfolio
              </SidebarItemLink>
            )}
            {canFindCategories && (
              <SidebarItemLink href='/categories'>
      ...
      <SidebarItem className='mt-auto w-full relative'>
        <UserNav />
      </SidebarItem>
      <SidebarSwitch />
    </Sidebar>
