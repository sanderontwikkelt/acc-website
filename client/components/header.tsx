...
const Header = ({ header }: { header: HeaderType }) => {
  ...
  return (
    <header
      ...
      className={cn(
        'fixed left-0 top-0 z-[99999] h-[6.25rem] w-full transition-colors',
        scrolled ? 'bg-accent' : 'bg-transparent'
      )}
    >
      <div className='relative z-50 flex h-full w-full max-w-[100vw] items-center px-[calc(1.875rem+max((100vw-(1300rem/16))/2,0px))]'>
      ...
      <MobileMenu
        navigation={[...getArray(header.navigation), ...getArray(header.links)]}
      />
      <div
        className={cn(
          '-z-1 absolute left-0 top-0 w-screen overflow-hidden bg-accent pt-[6.25rem] transition-all duration-300',
          ...
          <div className='flex h-full min-w-[50%] items-center justify-between'>
            {hoveredItem?.map(({ pathname, name }) => (
              <Link
                href={pathname}
                key={name}
                className='text-main/0.3 text-2xl transition-all hover:text-main hover:underline'
              >
                {name}
              </Link>
            ))}
