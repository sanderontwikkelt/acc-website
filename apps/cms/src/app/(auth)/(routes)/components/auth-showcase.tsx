...
import { loginEmail, loginGoogle } from "./actions";
...
export const EmailForm = () => {
  const [state, formAction] = useFormState(loginEmail, { message: "" });
  return (
    <form className="space-y-2" action={formAction}>
      <Input
        className={cn("w-full", state?.message ? "border-destructive" : "")}
        placeholder="E-mail"
        type="email"
        name="email"
      />
      {!!state?.message && (
        <p className="text-[0.8rem] font-medium text-destructive" role="status">
          {state?.message}
        </p>
      )}
      <EmailButton />
...
export async function AuthShowcase() {
  return (
    <div className="space-y-5">
      ...
      <form className="space-y-2" action={loginGoogle}>
